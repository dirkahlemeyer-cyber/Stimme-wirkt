...
const maraCoreLink='https://stimmeklar-web.vercel.app/mara';
for(const page of pages){
  const value=await readFile(new URL(`../${page}`,import.meta.url),'utf8');
  if(forbidden.test(value.replaceAll(maraCoreLink,''))) throw new Error(`${page} contains a forbidden previous-brand reference`);
...
for(const value of ['pricing-price','€ netto','300 Gesprächsminuten inklusive','249 € netto','20 € netto','Hier kündigen','mailto:support@stimmewirkt.de','Auftragsverarbeitung (AVV)','Mit Mara sprechen – bis zu 4 Minuten']){
...
if((faq.match(/<details/g)||[]).length<30||!faq.includes('demselben bestätigten Wissensstand')) throw new Error('FAQ is incomplete or not linked to the shared knowledge status');
